    ...
    def train(self, n_epochs=100, shuffle_batch=False):
        ...
        while (epoch < n_epochs):
            ...
            if prev_train_f1 is not None and train_f1 < prev_train_f1 and self.nonlinearity is None:
                ...
                print 'TEST', '=' * 40
                writef.write('Test' + '=' *40)
                writef.write('\n')
                test_f1, test_errors = self.compute_f1(self.data['test'])
                print '*** TEST_ERROR:', (1-test_f1)*100
                writef.write('*** TEST_ERROR:'+str((1-test_f1)*100))
                if True:
                    for i, pred in test_errors:
                        writef.write('\n')
                        writef.write('context: ' + self.to_words(self.data['test']['C'][i]))
                        writef.write('\n')
                        writef.write('question: '+ self.to_words([self.data['test']['Q'][i]]))
                        writef.write('\n')
